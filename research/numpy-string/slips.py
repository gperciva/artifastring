...
def play(x0, x1):
    ...
    vln.finger(x1, defs.K_finger, defs.R_finger)
    vln.bow(x0, 1.0, 0.2)
...
def go(x0, x1, fs=48000, a11b00_file=None, d1_file=None):
    defs.FS = fs
...
d1_file = open(thesis_filename("d1"), 'w')
...
for fs in FS_CANDIDATES:
    ...
    #    B00 = go(x0, 0, fs, a11b00_file, d1_file)
    #    #b00_file.write("%g\t%g\n" % (x0, B00) )
    #    B00s.append(B00)
    #    continue
    #print fs, min(B00s), max(B00s)
        ...
        #for x1 in numpy.linspace(0.99, 1.0, STEPS):
            ...
            #    continue
            #print '------', x0, x1
            go(x0, x1, fs, a11b00_file, d1_file)
        a11b00_file.write("\n")
        d1_file.write("\n")
    #b00_file.close()
a11b00_file.close()
